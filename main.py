...
parser = argparse.ArgumentParser('LLM40FD')
# basic config
parser.add_argument('--seed', type=int, default=2024, help='random seed')
parser.add_argument('--debug', type=int, default=0, help='debug mode')
parser.add_argument('--saving_name', type=str, default='normal', help='saving_name of dir')
...
def get_scores_labels(model, test_loader, config):
    import numpy as np
...
def main(config):
    ...
    for i in range(epochs):
        ...
        scores_list, scores = get_score_list(labels,scores, dataset1, dataset1, score_pth)
        for k,v in scores_list.items():
            print(f'{k}:{v:.4f}')

        save_scores_labels(scores, labels, dataset1, dataset1, score_pth)
        print(f'{dataset1} -> {dataset1} SCORES SAVED, epoch {i+1}')
    ...
    save_scores_labels(scores, labels, dataset1, dataset1, score_pth)
    print(f'{dataset1} -> {dataset1} SCORES SAVED')
    ...
    for dataset2 in dataset2_list:
        ...
        scores, labels = get_scores_labels(model, zere_shot_loader, config)
        scores = minmax_scale(scores)
        scores_list, scores = get_score_list(labels,scores,dataset1, dataset2, score_pth)
        # save scores_list
        for k,v in scores_list.items(): 
            print(f'{k}:{v:.4f}')

        save_scores_labels(scores, labels, dataset1, dataset2, score_pth)
        print(f'{dataset1} -> {dataset2} SCORES SAVED')
