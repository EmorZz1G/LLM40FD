...
def main(config):
    ...
    # TSETING D1
    scores, labels, says = get_scores_labels_outputs(model, test_loader, config)
    scores = minmax_scale(scores)
    scores_list, scores = get_score_list(labels,scores, dataset1, dataset1, None)
    for k,v in scores_list.items():
        print(f'{k}:{v:.4f}')

    top1 = np.quantile(scores,0.99)
    top1_idx = np.where(scores>top1)
    top1_says = says[top1_idx]
    print(top1_says)
    top_min_1 = np.quantile(scores,0.01)
    top_min_1_idx = np.where(scores<top_min_1)
    top_min_1_says = says[top_min_1_idx]
    print(top_min_1_says)
    ...
    for dataset2 in dataset2_list:
        ...
        scores, labels, says = get_scores_labels_outputs(model, zere_shot_loader, config)
        scores = minmax_scale(scores)
        ...
        print(scores.shape, says.shape)
        top1 = np.quantile(scores,0.99)
        top1_idx = np.where(scores>top1)
        top1_says = says[top1_idx]
        print(top1_says.shape,'YES')
        print(top1_says)
        top_min_1 = np.quantile(scores,0.01)
        top_min_1_idx = np.where(scores<top_min_1)
        top_min_1_says = says[top_min_1_idx]
        print(top1_says.shape,'NO')
        print(top_min_1_says)
