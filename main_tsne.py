...
def main(config):
    ...
    name = f'{dataset1}_{dataset1}{scores_list["best_f1"]:.4f}.pdf'
    ...
    for i, c, label in zip([0,1], colors, target_names):
        plt.scatter(X_2d[labels == i, 0], X_2d[labels == i, 1], c=c, label=label,s=5)
    plt.legend()
    ...
    for dataset2 in dataset2_list:
        ...
        name = f'{dataset1}_{dataset2}{scores_list["best_f1"]:.4f}.pdf'
        tsne = TSNE(n_components=2, random_state=0)
        X_2d = tsne.fit_transform(latents)
        # save img
        import matplotlib.pyplot as plt
